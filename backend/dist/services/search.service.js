...
exports.extractProductColors = exports.getFilterOptions = exports.searchProducts = void 0;
const Product_1 = __importDefault(require("../models/Product"));
const Category_1 = __importDefault(require("../models/Category"));
const Supplier_1 = __importDefault(require("../models/Supplier"));
const category_service_1 = __importDefault(require("./category.service"));
const searchProducts = async (filters) => {
    const { searchQuery, category, minPrice, maxPrice, supplier, color, size, sort = 'relevance', page = 1, limit = 20 } = filters;
    const query = {};
    const trimmedSearchTerm = searchQuery?.trim();
    const hasTextSearch = Boolean(trimmedSearchTerm);
    if (hasTextSearch) {
        const categoryIds = await Category_1.default.find({
    ...
    if (category) {
        try {
            const categoryDoc = await Category_1.default.findOne({
    ...
    if (supplier) {
        try {
            const supplierDoc = await Supplier_1.default.findOne({
...
const getFilterOptions = async (context) => {
    try {
        ...
        const allCategories = await category_service_1.default.getCategories();
        const categoriesFn = allCategories.map((cat) => {
        ...
        products.forEach(product => {
            ...
            if (product.supplierId && typeof product.supplierId === 'object') {
                suppliers.add(product.supplierId.name);
            }
            // Sizes
...
exports.extractProductColors = extractProductColors;
